...
def divide_dataset(source, train_percentage):
    ...
    dir_name = [source + '/dims_train', source + '/dims_validation']
...
def train(num_epoch, train_loader, validation_loader, model, optimizer, save_per_epoch, save_dir, use_cuda=False):
    ...
    for current_epoch in range(num_epoch):
        for batch_idx, (input_img, t_counts, t_cat, t_loc, t_orient, t_dims) in enumerate(train_loader):
            ...
            # pdb.set_trace()
            if args['use_cuda']:
                input_img, t_counts, t_cat, t_loc, t_orient, t_dims = input_img.cuda(), t_counts.cuda(), t_cat.cuda(), t_loc.cuda(), t_orient.cuda(), t_dims.cuda()

            input_img = inverse_xform_img(input_img, t_loc, t_orient, args['img_size'])
            t_loc, t_orient = default_loc_orient(actual_batch_size)
            real_sdf = render_obb_sdf((args['img_size'], args['img_size']), t_dims, t_loc, t_orient)
        ...
        if current_epoch % save_per_epoch == 0:
            ...
            model.save(f'{save_dir}/dims_model_{current_epoch}.pt')
            optimizers.save(f'{save_dir}/dims_opt_{current_epoch}.pt')
...
def validate(validation_loader, model, use_cuda=False):
    ...
    for batch_idx, (input_img, t_counts, t_cat, t_loc, t_orient,  t_dims) in enumerate(validation_loader):
        t_cat = torch.squeeze(t_cat)
        # Verify that we've got only one category in this batch
        # t_cat_0 = t_cat[0]
        # assert ((t_cat == t_cat_0).all())
...
if __name__ == '__main__':
    ...
    if args['use_cuda']:
        model = Dims(args['latent_size'], args['hidden_size'], input_channels).cuda()
    else:
        model = Dims(args['latent_size'], args['hidden_size'], input_channels)
