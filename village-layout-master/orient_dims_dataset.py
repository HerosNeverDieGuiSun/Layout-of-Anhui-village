...
class OrientDimsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        center_sides = eval(select_house['side_center_angle_2vercoordinate'])
        door_x1, door_x2 = int(center_sides[0][0]), int(center_sides[1][0])
        door_y1, door_y2 = int(center_sides[0][1]), int(center_sides[1][1])
        door_len = math.sqrt((door_x1-door_x2)**2 + (door_y1-door_y2)**2)

        sides = select_house['side']
        if abs(door_len - sides[0]) > abs(door_len - sides[1]):
            wall_len = sides[0]
        else:
            wall_len = sides[1]

        # (Normalize to [0, 1])
        door_len = door_len/256
        wall_len = wall_len/256

        dims = torch.Tensor([door_len, wall_len])           # 记录dims

        return input_img, existing_categories, cat, loc, orient, dims
...
if __name__ == "__main__":
    ...
    dataset = OrientDimsDataset(data_dir, block_category, num_house_categories)
    data_loader = DataLoader(dataset)
    data_iter = iter(data_loader)
    td_img, td_counts, td_cat, td_loc, td_orient, td_dims = next(data_iter)
